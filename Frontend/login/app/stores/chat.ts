...
export const useMessengerStore = defineStore('chat', {
  ...
  if (!userId || !token) {
    console.warn('No user or token found, SignalR not started')
    return
  }

  this.connection = new signalR.HubConnectionBuilder()
    .withUrl(`http://192.168.1.18/FinalChatBackend/hubs/messenger`, {
  ...
  if (
    ...
    async viewMessagePerson(userId: number, partnerId: number) {
      ...
      const res = await fetch(
        `http://192.168.1.18/FinalChatBackend/api/chat/view-person-message/${userId}/${partnerId}?page=1`,
        {
          method: 'GET',
          headers: {
